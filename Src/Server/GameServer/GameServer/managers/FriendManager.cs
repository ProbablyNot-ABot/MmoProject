...
namespace GameServer.Managers
{
    class FriendManager
    {
        ...
        public NFriendInfo GetFriendInfo(TCharacterFriend friend)
        {
            ...
            Log.InfoFormat("   {0}{1} GerFriendInfo : {2}:{3} Status:{4} ",this.Owner.Id,this.Owner.Info.Name,friendInfo.Id,friendInfo.friendInfo.Name,friendInfo.Status);
            return friendInfo;
        }
        ...
        public void PostProcess(NetMessageResponse message)
        {
            if (friendChanged)
            {
                Log.ErrorFormat("PostProcess > FriendManager: characterID:{0}:{1}", this.Owner.Id, this.Owner.Info.Name);
                this.InitFriends();
