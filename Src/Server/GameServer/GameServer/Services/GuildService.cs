...
namespace GameServer.Services
{
    class GuildService : Singleton<GuildService>
    {
        ...
        private void OnGuildCreate(NetConnection<NetSession> sender,GuildCreateRequest request)
        {
            ...
            Log.InfoFormat("OnGuildCreate: :CuildName:{0} character:[{1}]{2}", request.GuildName, character.Id, character.Name);
            sender.Session.Response.guildCreate = new GuildCreateResponse();
            if(character.Guild != null)
            {
                sender.Session.Response.guildCreate.Result = Result.Failed;
                sender.Session.Response.guildCreate.Errormsg = "已经有一个公会了";
                sender.SendResponse();
                return;
            }
            if(GuildManager.Instance.CheckNameExiested(request.GuildName))
            {
                sender.Session.Response.guildCreate.Result = Result.Failed;
        ...
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("OnGuildLeace: :character:{0}", character.Id);
            ...
            character.Guild.ExecuteAdmin(message.Commond, message.Target, character.Id);
            ...
            if(target != null)
            {
                ...
                target.Session.Response.guildAdmin.Commond = message;
                target.SendResponse();
            }

            sender.Session.Response.guildAdmin.Result = Result.Success;
            sender.Session.Response.guildAdmin.Commond = message;
            sender.SendResponse();
        }
    } 
}
