...
namespace GameServer.Services
{
    class FriendService : Singleton<FriendService>
    {
        ...
        private void OnFriendAddRequest(NetConnection<NetSession> sender, FriendAddRequest request)
        {
            ...
            Log.InfoFormat("FoewardRequest: :FromId:{0} FromName:{1} ToID{2} ToName:{3}", request.FromId, request.FromName, request.ToId, request.ToName);
        ...
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("OnFriendRemove: :character:{0} FriendRelectionID{1}",character.Id,request.Id);
