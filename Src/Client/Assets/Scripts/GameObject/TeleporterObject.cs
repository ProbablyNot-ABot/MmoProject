...
public class TeleporterObject : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (playerController != null && playerController.isActiveAndEnabled)
        {
            ...
            if (td == null)
            {
                Debug.LogErrorFormat("TeleporterObject: Character [{0}] Enter Teleporter [{1}], But TeleporterDefine not existed", playerController.character.Info.Name, ID);
                return;
            }
    ...
    void OnDrawGizmos()
    {
        ...
        UnityEditor.Handles.ArrowHandleCap(0,this.transform.position,transform.rotation,1f,EventType.Repaint);
    }
#endif
}
