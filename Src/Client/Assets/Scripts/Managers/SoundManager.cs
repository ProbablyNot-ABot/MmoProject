...
namespace Managers
{
    class SoundManager : MonoSingleton<SoundManager>
    {
        ...
        public void PlayMusic(string name)
        {
            ...
            Debug.Log("音乐的路径是" + MusicPath + name);
            if(clip == null)
            {
                Debug.LogWarningFormat("PlayerMusic : {0} not existed", name);
                return;
            }
        ...
        public void PlaySound(string name)
        {
            ...
            Debug.Log("音效的路径是" + SoundPath + name);
            if (clip == null)
            {
                Debug.LogWarningFormat("PlayerSound : {0} not existed", name);
                return;
            }
