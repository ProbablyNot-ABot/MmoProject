...
namespace Services
{
    class GuildService : Singleton<GuildService>,IDisposable
    {
        ...
        /// 发送加入公会请求
        /// </summary>
        /// <param name="guildInd"></param>
        public void SendGuildJoinRequest(int guildInd)
        {
            ...
            message.Request.guildJoinReq.Apply.GuildId = guildInd;
            NetClient.Instance.SendMessage(message);
        }
        ...
        internal void SendAdminCommand(GuildAdminCommand command, int characterId)
        {
            ...
            message.Request.guildAdmin.Commond = command;
        ...
        {
            Debug.LogFormat("OnGuildAdmin: {0},{1}", message.Commond,message.Result);
            MessageBox.Show(string.Format("执行操作:{0} 结果:{1} {2}",message.Commond,message.Result,message.Errormsg));
        }
    }
}
