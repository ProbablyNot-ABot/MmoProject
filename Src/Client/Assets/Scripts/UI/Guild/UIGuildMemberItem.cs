...
public class UIGuildMemberItem : ListView.ListViewItem
{
    ...
    public void SetGuildMemberInfo(NGuildMemberInfo info)
    {
        ...
        if(this.status != null) this.status.text = this.Info.Status == 1 ? "ÔÚÏß" : TimeUtil.GetTime(this.Info.lastTime).ToShortDateString();
        if(this.status != null) this.status.color = this.Info.Status == 1 ? Color.green : Color.gray;
    }
}
